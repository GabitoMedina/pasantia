...
$_SESSION["errors"] = $errors;
	?>
<script>
	window.location="index.php?view=proform1";
</script>
<?php
}
...
//////////////////////////////////
		if($process==true){
			$sellprfm = new SellprfmData();
			$sellprfm->user_id = $_SESSION["user_id"];

			$sellprfm->total = $_POST["total"];
			$sellprfm->discount = $_POST["discount"];


			 if(isset($_POST["client_id"]) && $_POST["client_id"]!=""){
			 	$sellprfm->person_id=$_POST["client_id"];
 				$s = $sellprfm->add_with_client();
			 }else{
 				$s = $sellprfm->add();
			 }
...
////////////////////
print "<script>window.location='index.php?view=onesellprfm&id=$s[1]';</script>";
		}
	}
}
